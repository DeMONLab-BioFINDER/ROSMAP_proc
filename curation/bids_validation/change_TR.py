...
def main(paths):
    ...
    with ThreadPoolExecutor(max_workers=48) as executor:
        results = list(filter(None, executor.map(process_file, paths)))
...
# Check if script is executed with correct number of args
    if len(sys.argv) != 2:
        print("Correct syntax: python [this_script.py] [df.csv]")
        sys.exit(1)
    
    # Load the dataframe
    df = pd.read_csv(sys.argv[1])

    # Filter entries in the 'new_path' column that end with '_bold.nii.gz'
    filtered_paths = df[
    df['new_path'].str.endswith('_bold.nii.gz') & 
    ~df['new_path'].str.contains('PhysioLog')
]['new_path'].str.replace("../raw/", "", regex=False)


    # Pass the filtered paths to the main function
    main(filtered_paths.to_list())
