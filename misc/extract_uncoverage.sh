...
### 
# to extract time series from the denoised BOLD files using the individual atlas and the labels file. with no min coverage threshold
###
...
LABELS="atlas-Schaefer400TianS2Cereb/MNINLin6Asym/atlas-Schaefer400TianS2Cereb_dseg.tsv"
...
extract_ts() {
    ...
    if [[ -z "$denoised_bold" ]]; then
        echo "ERROR: Could not find denoised BOLD file: $base" >&2
        return 1
    fi
    ...
    individual_atlas="individual_atlases/${sid}_${session}_400atlases/atlas-Schaefer400TianS2Cereb_space-MNI152NLin6Asym_res-2_dseg.nii.gz"

    python "${script_dir}/extract_uncoverage.py" "$individual_atlas" "$LABELS" "$denoised_bold" "$output"
